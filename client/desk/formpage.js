...
module.exports = class FormPage extends Page {
    constructor(doctype) {
        ...
        this.form.on('save', async (params) => {
            let route = dataent.router.getRoute();
            if (this.form.doc.name && !(route && route[2] === this.form.doc.name)) {
                await dataent.router.setRoute('edit', this.form.doc.doctype, this.form.doc.name);
                dataent.ui.showAlert({message: 'Added', color: 'green'});
            }
        });

        this.form.on('delete', async (params) => {
            this.hide();
            await dataent.router.setRoute('list', this.form.doctype);
        });
    }
    ...
    async show(params) {
        ...
        try {
            ...
            dataent.desk.setActiveDoc(this.form.doc);
        } catch (e) {
