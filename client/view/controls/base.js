...
class BaseControl {
    constructor({field, parent, form}) {
        ...
        if (!this.fieldname) {
            this.fieldname = dataent.slug(this.label);
        }
        if (!this.parent) {
        ...
        if (this.template) {
            this.wrapper = dataent.ui.add('div', 'field-template', this.parent);
            this.renderTemplate();
        } else {
            this.make();
        }
    ...
    makeInputContainer(className = 'form-group') {
        this.inputContainer = dataent.ui.add('div', className, this.parent);
    }

    makeLabel(labelClass = null) {
        this.labelElement = dataent.ui.add('label', labelClass, this.inputContainer, this.label);
    ...
    makeInput(inputClass='form-control') {
        this.input = dataent.ui.add('input', inputClass, this.getInputParent());
    ...
    makeDescription() {
        if (this.description) {
            this.description_element = dataent.ui.add('small', 'form-text text-muted',
