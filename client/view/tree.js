...
const Tree = require('dataentjs/client/components/tree');
...
module.exports = class BaseTree extends BaseList {
    ...
    makeBody() {
        if (!this.body) {
            ...
            this.body = dataent.ui.add('div', 'tree-body', this.parent);
    ...
    renderTree(rootLabel) {
        ...
        this.treeWrapper = dataent.ui.create(`
            <f-tree>
        ...
        dataent.ui.on(this.treeWrapper, 'tree-node-expand', 'f-tree-node', async (e, treeNode) => {
            ...
            if (!treeNode.props.children) {
                ...
                for (let child of children) {
                    const childNode = dataent.ui.create(this.getTreeNodeHTML(child));
        ...
        dataent.ui.on(this.treeWrapper, 'tree-node-action', 'f-tree-node', (e, treeNode) => {
    ...
    edit(name) {
        dataent.desk.showFormModal(this.doctype, name);
    }

    async addChildNode(name) {
        const newDoc = await dataent.getNewDoc(this.doctype);
        const formModal = await dataent.desk.showFormModal(this.doctype, newDoc.name);
    ...
    getActionButtonsHTML() {
        return [
            { id: 'edit', label: dataent._('Edit') },
            { id: 'addChild', label: dataent._('Add Child') },
            // { id: 'delete', label: dataent._('Delete') },
        ].map(button => {
