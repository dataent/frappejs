...
module.exports = class BaseList extends Observable {
    ...
    init() {
        ...
        dataent.db.on(`change:${this.doctype}`, (params) => {
            this.refresh();
        });
    }
    ...
    makeBody() {
        if (!this.body) {
            ...
            this.body = dataent.ui.add('div', 'list-body', this.parent);
    ...
    getRow(i) {
        if (!this.rows[i]) {
            let row = dataent.ui.add('div', 'list-row row no-gutters', this.body);
    ...
    async showItem(name) {
        if (this.meta.print) {
            await dataent.router.setRoute('print', this.doctype, name);
        } else {
            await dataent.router.setRoute('edit', this.doctype, name);
        }
    }
    ...
    selectDefaultRow() {
        if (!dataent.desk.body.activePage && this.rows.length) {
    ...
    makeToolbar() {
        ...
        this.btnNew = this.page.addButton(dataent._('New'), 'btn-primary', async () => {
            await dataent.router.setRoute('new', this.doctype);
        });

        this.btnDelete = this.page.addButton(dataent._('Delete'), 'btn-secondary hide', async () => {
            await dataent.db.deleteMany(this.doctype, this.getCheckedRowNames());
            await this.refresh();
        });

        this.btnReport = this.page.addButton(dataent._('Report'), 'btn-outline-secondary hide', async () => {
            await dataent.router.setRoute('table', this.doctype);
        });
    ...
    makeSearch() {
        this.toolbar = dataent.ui.add('div', 'list-toolbar', this.parent);
    ...
    bindKeys() {
        ...
        keyboard.bindKey(this.body, 'x', async (e) => {
            ...
            if (activeListRow && activeListRow.docName) {
                e.preventDefault();
                await dataent.db.delete(this.doctype, activeListRow.docName);
    ...
    makeMoreBtn() {
        this.btnMore = dataent.ui.add('button', 'btn btn-secondary hide', this.parent, 'More');
    ...
    setActiveListRow(name) {
        ...
        if (!name) {
            ...
            name = dataent.desk.activeDoc && dataent.desk.activeDoc.name;
        }
