...
module.exports = class HTTPClient extends Observable {
    constructor({ server, protocol = 'http' }) {
        ...
        dataent.config.serverURL = this.getURL();

        // if the backend is http, then always client!
        dataent.isServer = false;
    ...
    async getAll({ doctype, fields, filters, start, limit, sort_by, order }) {
        ...
        url = url + "?" + dataent.getQueryString({
    ...
    async fetch(url, args) {
        ...
        let response = await dataent.fetch(url, args);
    ...
    getHeaders() {
        ...
        if (dataent.session && dataent.session.token) {
            headers.token = dataent.session.token;
        };
        return headers;
    }
