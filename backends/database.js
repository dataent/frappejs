...
module.exports = class Database extends Observable {
    ...
    async migrate() {
        for (let doctype in dataent.models) {
    ...
    async getColumnDiff(doctype) {
        ...
        const validFields = dataent.getMeta(doctype).getValidFields({ withChildren: false });
    ...
    async get(doctype, name=null, fields = '*') {
        ...
        if (meta.isSingle) {
            ...
            if (!name) {
                throw new dataent.errors.ValueError('name is mandatory');
            }
    ...
    async updateSingle(meta, doc, doctype) {
        ...
        for (let field of meta.getValidFields({withChildren: false})) {
            ...
            if (value) {
                let singleValue = dataent.newDoc({
                    doctype: 'SingleValue',
                    parent: doctype,
    ...
    prepareChild(parenttype, parent, child, field, idx) {
        if (!child.name) {
            child.name = dataent.getRandomString();
        }
        child.parent = parent;
    ...
    getKeys(doctype) {
        return dataent.getMeta(doctype).getValidFields({ withChildren: false });
    }
    ...
    async delete(doctype, name) {
        ...
        // delete children
        let tableFields = dataent.getMeta(doctype).getTableFields();
        for (let field of tableFields) {
